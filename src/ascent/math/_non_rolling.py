...
class GeometricBrownianMotion(StochasticModel):
    ...
    def log_likelihood(self, X: np.ndarray) -> float:
        ...
        term3 = -np.sum((log_returns - expected_mean) ** 2) / (2 * self.params.sigma**2 * DELTA_T)
    ...
    def simulate(self, N: int, N_simulated: int, X_0: float) -> np.ndarray:
        ...
        for i in range(1, N):
            X_simulated[:, i] = X_simulated[:, i - 1] * np.exp(
                ...
                + self.params.sigma * np.sqrt(DELTA_T) * np.random.normal(0, 1, N_simulated)
            )
...
class OrnsteinUhlenbeck(StochasticModel):
    ...
    @staticmethod
    def f(u: float, x: float, mu: float, sigma: float, theta: float, r: float = 0.01) -> float:
        """
    ...
    @staticmethod
    def g(u: float, x: float, mu: float, sigma: float, theta: float, r: float = 0.01) -> float:
        """
    ...
    def _compute_integral(
        ...
        if not use_analytical:
            all_scalar = (
                np.isscalar(effective_alpha) and np.isscalar(beta) and np.isscalar(scale_factor)
            )
            ...
            for idx in np.ndindex(alpha_arr.shape):
                result[idx] = (
                    ...
                    * quad(
                        lambda u, a=alpha_arr[idx], b=beta_arr[idx]: (
                            u**a * np.exp(b * u - u**2 / 2)
                        ),
                        0,
                        np.inf,
                    )[0]
        ...
        if np.any(non_zero_mask):
            ...
            if np.any(D_is_posinf):
                ...
                non_zero_result[D_is_posinf] = np.where(prefactor_is_zero, np.nan, np.inf)

            if np.any(D_is_neginf):
                prefactor_is_zero = log_prefactor[D_is_neginf] == -np.inf
                non_zero_result[D_is_neginf] = np.where(prefactor_is_zero, np.nan, -np.inf)
            ...
            if np.any(D_is_finite):
                ...
                if np.any(finite_overflow):
                    ...
                    signs = np.sign(prefactor[finite_overflow]) * np.sign(D_vals[indices])
    ...
    def V(
        ...
        x_arr, mu_arr, sigma_arr, theta_arr, r_arr, c_arr, exit_level_arr = np.broadcast_arrays(
    ...
    def get_optimal_exit_level(
        ...
        if len(mu_v) == 1:
            ...
            x0_spread = np.array([x_grid_spread[np.argmin(f_search)]])
        else:
            ...
            # Filter: f(x) > 0 and f'(x) > 0 (both must be positive)
            f_search = np.where((f_grid > 0) & (fp_grid > 0), f_grid, np.inf)
            x0_spread = x_grid_spread[np.argmin(f_search, axis=0), np.arange(len(mu_v))]

        # Newton's method in spread space (theta=0)
        x_spread = x0_spread
        active = np.ones(len(x_spread), dtype=bool)
        for _ in range(max_iter):
            if not np.any(active):
                break
            f_val = f_exit_level(x_spread[active], mu_v[active], sigma_v[active], 0, r, c)
            fp_val = f_prime_exit_level(x_spread[active], mu_v[active], sigma_v[active], 0, r, c)
            safe = (fp_val != 0) & np.isfinite(fp_val) & np.isfinite(f_val)
            step = np.zeros_like(f_val)
            np.divide(f_val, fp_val, out=step, where=safe)
            x_spread[active] = x_spread[active] - step
            active_idx = np.flatnonzero(active)
            # NaN out elements that hit invalid values
            x_spread[active_idx[~safe]] = np.nan
        ...
        # Convert from spread space back to absolute positions by adding theta
        result[valid] = x_spread + theta_v
        return result
