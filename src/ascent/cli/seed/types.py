...
def seed_types(client: Any, ctx: dict) -> None:
    ...
    client.create_exchange_type(
        ...
        description="Options exchange for listed options contracts",
    )
    client.create_exchange_type(
        name="DERIVATIVES",
        display_name="Derivatives",
        description="Multi-product derivatives exchange",
    )

    # --- Strategy Types ---
    for name, display_name, desc in [
        ("PAIRS_TRADING", "Pairs Trading", "Statistical arbitrage between two correlated assets"),
        ("MOMENTUM", "Momentum", "Trend-following strategy based on price momentum"),
        (
            "MEAN_REVERSION",
            "Mean Reversion",
            "Strategy based on price reverting to a long-run mean",
        ),
        (
            "STATISTICAL_ARBITRAGE",
            "Statistical Arbitrage",
            "Multi-asset quantitative strategy exploiting statistical mispricings",
        ),
        (
            "MARKET_MAKING",
            "Market Making",
            "Providing liquidity by quoting two-sided markets and capturing bid-ask spread",
        ),
        (
            "TREND_FOLLOWING",
            "Trend Following",
            "Systematic strategy that enters positions in the direction of established trends",
        ),
        (
            "BREAKOUT",
            "Breakout",
            "Strategy that enters when price breaks through support or resistance levels",
        ),
        (
            "GRID_TRADING",
            "Grid Trading",
            "Places buy and sell orders at predefined price intervals around a base price",
        ),
        (
            "VOLATILITY",
            "Volatility",
            "Strategy that trades volatility levels, skew, or term structure",
        ),
        (
            "CARRY_TRADE",
            "Carry Trade",
            "Strategy exploiting yield differentials between instruments or funding rates",
        ),
    ]:
        client.create_strategy_type(name=name, display_name=display_name, description=desc)
    strategy_types = client.get_strategy_types()
    ctx["strategy_type_by_name"] = {t["name"]: t for t in strategy_types}
