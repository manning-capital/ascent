...
Runs synchronous SQLAlchemy on a threadpool (``asyncio.to_thread``) so we
keep the async port contract without a full async-SQLAlchemy migration.
"""
...
class SqlAlchemyTradeRepository(TradeRepository):
    ...
    async def get(self, trade_id: uuid.UUID) -> Trade | None:
        return await asyncio.to_thread(self._get_sync, trade_id)

    async def list_non_terminal_for_strategy(self, strategy_id: uuid.UUID) -> list[Trade]:
        return await asyncio.to_thread(self._list_non_terminal_sync, strategy_id)

    async def list_open_for_strategy(self, strategy_id: uuid.UUID) -> list[Trade]:
        return await asyncio.to_thread(self._list_open_sync, strategy_id)
    ...
    async def set_leg_prices(
        ...
        *,
        ...
        await asyncio.to_thread(
            self._set_leg_prices_sync, leg_id, entry_price, exit_price, realized_pnl
        )

    async def set_entry_order(self, leg_id: uuid.UUID, order_id: uuid.UUID) -> None:
        await asyncio.to_thread(self._set_entry_order_sync, leg_id, order_id)

    async def set_exit_order(self, leg_id: uuid.UUID, order_id: uuid.UUID) -> None:
        await asyncio.to_thread(self._set_exit_order_sync, leg_id, order_id)

    # ---------------- sync internals ----------------

    def _get_sync(self, trade_id: uuid.UUID) -> Trade | None:
    ...
    def _list_non_terminal_sync(self, strategy_id: uuid.UUID) -> list[Trade]:
        terminal_ids = [
    ...
    def _list_open_sync(self, strategy_id: uuid.UUID) -> list[Trade]:
    ...
    def _set_state_sync(
        ...
        with Session(bind=self._sf.kw["bind"]) as db:
            ...
            # Offset the status timestamp by 1µs if it would collide with the
            # trade's last status row.
            timestamp = at
            last = (
                db.execute(
    ...
    def _set_entry_order_sync(self, leg_id: uuid.UUID, order_id: uuid.UUID) -> None:
        with Session(bind=self._sf.kw["bind"]) as db:
            leg = db.get(TradeLegRow, leg_id)
            if leg is None:
                return
    ...
    def _set_exit_order_sync(self, leg_id: uuid.UUID, order_id: uuid.UUID) -> None:
        with Session(bind=self._sf.kw["bind"]) as db:
            leg = db.get(TradeLegRow, leg_id)
            if leg is None:
                return
...
_ = (OrderRow, replace)  # keep imports tidy while TYPE_CHECKING isn't used
