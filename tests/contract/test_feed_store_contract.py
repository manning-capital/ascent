...
so any backend going through JSON must preserve the columns and values the
strategy context builder depends on (``timestamp``, ``instrument_id``,
...
def _eav_df() -> pd.DataFrame:
    """The exact shape feeds publish — matches ``feed.py`` in the repo root."""
    return pd.DataFrame(
        [
            {
...
class TestLatestRoundTrip:
    ...
    async def test_put_and_get_latest(self, feed_store):
        ...
        # Every column the context builder needs must be present and values intact.
        assert set(fetched.columns) >= {
            "timestamp",
            "instrument_id",
            "attribute_id",
            "attribute_value",
    ...
    async def test_put_overwrites_previous(self, feed_store):
        feed_id = uuid.uuid4()
        await feed_store.put_latest(feed_id, _eav_df(), produced_at=NOW)
        new_df = pd.DataFrame(
            [
                {
        ...
        assert fetched["attribute_value"].iloc[0] == 999.0
...
class TestBatchFetch:
    ...
    async def test_get_latest_many_returns_mapping_with_none_for_missing(self, feed_store):
        have = uuid.uuid4()
        missing = uuid.uuid4()
        await feed_store.put_latest(have, _eav_df(), produced_at=NOW)
...
class TestHistoricalUpsert:
    ...
    async def test_upsert_returns_rowcount(self, feed_store):
        ...
        n = await feed_store.upsert(uuid.uuid4(), "instrument_attribute", _eav_df())
        assert n == 2
